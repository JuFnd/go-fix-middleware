...
func GetApi(c *usecase.Core, l *slog.Logger, cfg *configs.CommentCfg) *API {
	...
	mx.Handle("/api/v1/comment", api.mw.GetResponse(http.HandlerFunc(api.Comment), l))
	mx.Handle("/api/v1/comment/add", api.mw.GetResponse(http.HandlerFunc(api.AddComment), l))
